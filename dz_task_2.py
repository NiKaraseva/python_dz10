# Доп условие: если передаётся только одна сторона считаем, что треугольник равносторонний.
# Если передаётся 2 стороны – что треугольник равнобедренный.
    ...
    def check_triangle(self):
        if self.side_1 > 0 and self.side_2 > 0 and self.side_3 > 0:
            if self.side_1 + self.side_2 > self.side_3 \
                ...
                raise ValueError("Invalid size of sides: triangle does not exist")
        else:
            raise ValueError("Invalid size of sides: it cannot be less than 0")
...
if __name__ == '__main__':
    ...
    print(f'tr3 = {tr3.check_triangle()}')
